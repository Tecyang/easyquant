...
class MainEngine:
    ...
    def load(self, names, strategy_file):
        with self.lock:
            mtime = os.path.getmtime(os.path.join(
                '/Users/tecyang/code/other/easyquant/strategies', strategy_file))

            # 是否需要重新加载
            reload = False

            strategy_module_name = os.path.basename(strategy_file)[:-3]
            def new_module(strategy_module_name): return importlib.import_module(
    ...
    def load_strategy(self, names=None):
        ...
        s_folder = 'strategies'
        self._names = names
        strategies = os.listdir(
            '/Users/tecyang/code/other/easyquant/'+s_folder)
